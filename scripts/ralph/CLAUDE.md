...
2. Read `scripts/ralph/progress.txt` -- it contains only recent story entries.
   ...
   **CRITICAL**: Only read the last ~50 lines (use offset/limit) to save tokens.
...
7. Explore the codebase -- **read every file you will modify** before writing any code.
   ...
   **CRITICAL**: Exclude `node_modules`, `dist`, `.git`, and `scripts/ralph/flowchart` from all searches.
...
12. When all gates pass, smoke exits 0, and reviewer returns no Critical items:
    ...
    **CRITICAL**: Never set `passes: true` without committing. The Reviewer uses `git diff HEAD~1`.
