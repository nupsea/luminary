...
IS_ARRAY=$(python3 -c "import json; data=json.load(open('$SMOKE_TMP/s75_sections.json')); print('yes' if isinstance(data, list) else 'no')" 2>/dev/null || echo "no")
if [ "$IS_ARRAY" != "yes" ]; then
  echo "FAIL: GET /summarize/${DOC_ID}/sections did not return a JSON array"
  cat $SMOKE_TMP/s75_sections.json
  exit 1
fi

COUNT=$(python3 -c "import json; print(len(json.load(open('$SMOKE_TMP/s75_sections.json'))))" 2>/dev/null || echo "0")
