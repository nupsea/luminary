...
TOC_STATUS=$(curl -s -o /dev/null -w "%{http_code}" "${BASE}/documents/${UNKNOWN_ID}/epub/toc")
...
CHAPTER_STATUS=$(curl -s -o /dev/null -w "%{http_code}" "${BASE}/documents/${UNKNOWN_ID}/epub/chapter/0")
