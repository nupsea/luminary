...
async def test_tag_migration_merges_duplicates(test_db):
    ...
    async with factory() as session:
        ...
        session.add(NoteModel(
            id=note1_id, content="Note 1", tags=["Python"],
            document_id=None, group_name="manual",
        ))
        session.add(NoteModel(
            id=note2_id, content="Note 2", tags=["python"],
            document_id=None, group_name="manual",
        ))

        # Create canonical tags
        session.add(CanonicalTagModel(
            id="Python", display_name="Python", parent_tag=None,
            note_count=1, created_at=datetime.now(UTC),
        ))
        session.add(CanonicalTagModel(
            id="python", display_name="python", parent_tag=None,
            note_count=1, created_at=datetime.now(UTC),
    ...
    async with factory() as session:
        ...
        for nid in [note1_id, note2_id]:
            note = (await session.execute(
                select(NoteModel).where(NoteModel.id == nid)
            )).scalar_one()
...
async def test_collection_migration_merges_duplicates(test_db):
    ...
    async with factory() as session:
        ...
        session.add(NoteCollectionModel(
            id=col1_id, name="My Notes", color="#6366F1",
            created_at=datetime.now(UTC), updated_at=datetime.now(UTC),
        ))
        session.add(NoteCollectionModel(
            id=col2_id, name="MY-NOTES", color="#8B5CF6",
            created_at=datetime.now(UTC), updated_at=datetime.now(UTC),
        ))

        # Create notes
        session.add(NoteModel(
            id=note1_id, content="Note 1", tags=[],
            document_id=None, group_name="manual",
        ))
        session.add(NoteModel(
            id=note2_id, content="Note 2", tags=[],
            document_id=None, group_name="manual",
        ))
        ...
        await session.execute(text(
            ...
            " VALUES (:id, :nid, :cid, :at)"
        ), {"id": str(uuid.uuid4()), "nid": note1_id, "cid": col1_id,
            "at": datetime.now(UTC).isoformat()})
        await session.execute(text(
            "INSERT INTO note_collection_members (id, note_id, collection_id, added_at)"
            " VALUES (:id, :nid, :cid, :at)"
        ), {"id": str(uuid.uuid4()), "nid": note2_id, "cid": col1_id,
            "at": datetime.now(UTC).isoformat()})
        await session.execute(text(
            "INSERT INTO note_collection_members (id, note_id, collection_id, added_at)"
            " VALUES (:id, :nid, :cid, :at)"
        ), {"id": str(uuid.uuid4()), "nid": note2_id, "cid": col2_id,
            "at": datetime.now(UTC).isoformat()})
        await session.commit()
...
async def test_sync_tag_index_normalizes(test_db):
    ...
    async with factory() as session:
        session.add(NoteModel(
            id=note_id, content="Test", tags=["Machine Learning"],
            document_id=None, group_name="manual",
        ))
        await session.commit()
    ...
    async with factory() as session:
        rows = (await session.execute(
            select(NoteTagIndexModel.tag_full).where(
...
async def test_autocomplete_normalizes_query(test_db):
    ...
    async with factory() as session:
        session.add(CanonicalTagModel(
            id="machine-learning", display_name="machine-learning",
            parent_tag=None, note_count=5, created_at=datetime.now(UTC),
        ))
        await session.commit()
