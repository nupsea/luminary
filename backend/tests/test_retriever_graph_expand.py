...
async def test_graph_expand_resolves_aliases():
    ...
    graph = _mock_graph_with_aliases({"sherlock holmes": "Holmes|Mr. Holmes|detective"})
...
async def test_graph_expand_token_cap():
    ...
    extractor = _mock_extractor([{"name": f"name{i}", "type": "PERSON"} for i in range(3)])
    long_aliases = "|".join([f"alias{i}_{j} extra word" for i in range(3) for j in range(5)])
    graph = _mock_graph_with_aliases({f"name{i}": long_aliases for i in range(3)})
...
async def test_retrieve_graph_expand_true_passes_expanded_query():
    ...
    with (
        ...
        await retriever.retrieve("What happened?", document_ids=["doc-1"], k=5, graph_expand=True)
