"""KuzuViewRepo: cross-cutting read-side aggregator for the graph viz API.
...
ViewRepo reads across the entity / tech / note layers to build the
viz payload consumed by `pages/Viz.tsx`. It depends on
`KuzuConnection` for the raw connection and `KuzuTechRepo` for the
internal helpers `_get_tech_relation_edges` and `_get_diagram_nodes_by_type`.
"""
...
class KuzuViewRepo:
    def __init__(
        ...
        tech: KuzuTechRepo,
        prereq: KuzuPrereqRepo,
        concept: KuzuConceptRepo,
    ) -> None:
