...
LanceDB stores dense embeddings for chunks and notes. Built on Apache Arrow, it is embedded (no server), supports fast cosine similarity search, and handles incremental upserts efficiently. Vectors are 384-dimensional, generated by the BAAI/bge-small-en-v1.5 model via SentenceTransformer.
...
chunk_node          Split into overlapping segments (content-type-aware sizes)
  |
  v
embed_node          Generate 384-dim embeddings (BAAI/bge-small-en-v1.5)
  |
  v
...
User's machine
  ...
  |           +-- models/           (bge-small, GLiNER model cache)
