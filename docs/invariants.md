...
Reference enrichment sent a HEAD request to every link the model suggested, after every ingest and whenever the references panel opened: ~25 outside hosts per three-document run, with web search off. The test for "no HTTP when the provider is none" mocked out the method that sent them. Link checks go through `reference_validator.link_checks_enabled()`, which is off while `WEB_SEARCH_PROVIDER` is `none`; references stay unchecked (`is_valid` null) until it is on. Fetching a URL the user submits for ingest is asked for. `tests/test_reference_validator.py::test_no_link_check_without_web_access` and `tests/test_reference_enricher.py::test_no_http_calls_when_provider_none` fail CI if either entry point reaches `httpx` with web access off.
...
`OLLAMA_NUM_PARALLEL` is the width; a wider app semaphore only moves the wait into the runtime queue, where it counts against the caller's timeout. Four callers over 1 slot share 54.7 tok/s (done at 4.5/8.9/13.5/17.8s); over 2 slots, 99.3 (M3 Pro, llama3.2). Size semaphores at the slot count, and admission caps every background call at it with or without a question in flight: uncapped, four background calls sat in Ollama's queue ahead of a `/qa` that timed out, and the defer bound, which lifts only the interactive reserve, had forced each waiter past the width. Waiters are admitted in arrival order, and a background-routed call someone is waiting on (a request's own call, or one a document needs before `complete`) runs under `awaited()` and goes first: `background=True` decides routing, not priority, and without it a two-line note waited minutes behind other documents' enrichment (#142) and `suggest-tags` behind twenty queued calls. Default 1, since a slot costs a full KV cache; every install path sizes from physical RAM, never a profile name (`>= 24 -> 2, else 1`), and the desktop `supervisor.rs` passes the same number to Ollama and the backend. The runtime does not preempt, but a client cancel frees the slot, so only work with a real fallback may yield to a waiting question (`run_yielding_to_interactive`, suggestions); enrichment may not. Prompt eval is ~0.4s against ~16s decode, so reach for fewer calls first: `web_refs` cost ~50 of DDIA's ~80 enrichment minutes. `test_installer_models.py::test_the_serving_width_band_agrees_across_every_install_path` and `test_background_yields_the_slot_to_a_waiting_question.py` and `test_llm_admission.py::test_background_never_exceeds_the_serving_width`, `::test_background_waiters_are_admitted_in_arrival_order` guard it.
...
On Fedora 44 and Bluefin (Mesa 26) 0.13.8 opened a blank window: the backend logged `ready`, and WebKit's web process printed `Could not create default EGL display: EGL_BAD_PARAMETER. Aborting...` and died. AppRun puts the bundle's `usr/lib` ahead of the system's on the library path, but Mesa, EGL and the Vulkan drivers always come from the host, so they resolve their own dependencies against whatever the bundle carries: Ubuntu 22.04's libwayland 1.20 lacks `wl_display_create_queue_with_name`, `wl_display_dispatch_queue_timeout` and `wl_fixes_interface`, `libEGL_mesa` failed to load, and glvnd had no vendor. The build machine's own Mesa matches its libwayland, so xvfb on the build base cannot show it, and tauri's pinned linuxdeploy ignores `LINUXDEPLOY_EXCLUDED_LIBRARIES`. `scripts/desktop/prune_appimage.sh` removes the host-owned libraries after the build; `scripts/desktop/check_appimage_host_libs.sh` fails `desktop-installers.yml` when any Mesa driver on current Fedora, Arch, Ubuntu or Debian cannot resolve with the bundle first, and runs once against the unpruned image, where it must fail. `verify_installed.sh` also requires the shell's `page loaded` line, since `ready` passed the dead window.
