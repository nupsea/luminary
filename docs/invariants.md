...
- **Default 1.** A slot costs a full `OLLAMA_NUM_CTX` KV cache (896 MiB for a 3B model), and under 24GB the second competes with the 7B vision model for residency. Every install path sizes from **physical RAM, never from the profile name** — `install.sh`, `install.ps1`, `bootstrap.sh` and `supervisor.rs` all read `>= 24 -> 2, else 1`. The auto path never exceeds 2, and 4 is opt-in through `.env` alone: keying it to `LUMINARY_PROFILE=performance` was safe only while that profile had to be chosen by hand, and it is sized from RAM now. Keying width to the profile is what broke it once already — these values were written when `public` meant "under 24GB" and took one slot, so the catch-all meant 24GB+ and took two; retiring `public` left the 24GB+ line catching 16GB laptops, the value unchanged and the band beneath it moved. `tests/test_installer_models.py::test_the_serving_width_band_agrees_across_every_install_path` fails CI if any path sizes from the profile again, or sets 4 automatically.
...
**I-39. A model's residency is what the runtime reports for it, never a process RSS, and every site that sizes the machine is sized from the same band.**
`scripts/mem_profile.py` records both bases for one profiled ingest, deliberately: `peak_rss_mb` 7673.3, `peak_backend_mb` 3270.5, `peak_ollama_reported_mb` 3290.4. Subtracting the backend from the peak leaves 4402.8MB, and that remainder was read as a model's footprint — `qwen3.5:4b` at "4.31GB" against the 3.21GB Ollama reports for it, and `qwen2.5vl:7b` at "8.77GB" against 6.81GB. Both are ~30% high, and the second has no measurement behind it at all: that run never loaded a second model (`max_models_loaded: 1`). The pair then read as 13.07GB, or 102% of a 16GB machine, and `MAX_RESIDENT["standard"]` shipped at one resident model. Measured from `/api/ps` with both resident at once, the pair is 3.21 + 6.81 = **10.02GB**.

On unified memory the runner maps weights the kernel may or may not charge to the process, so `rss.ollama` and Ollama's own accounting disagree **by design** — which is why `mem_profile.py` reports both and its docstring states that a number appearing only one way is a measurement artifact, not a finding. RSS is the right basis for our own process and the wrong one for a model, so a sum across the two measures nothing. The same pass read `peak_ollama_reported_mb` — 3290.4, an MB field — as "3.29GB"; **every `*_mb` field in that summary is MB**, including `peak_backend_mb`, whose 3270.5 is 3.19GB.

**The runner count is a permission, and a permission is not a budget.** `MAX_RESIDENT` says how many runners may load; `fits_together` weighs them, at half the machine (`_RESIDENT_SET_FRACTION`), and it is the budget that protects a small desktop. At the 16GB floor the two settings resolve identically — the 10.02GB pair exceeds the 8GB budget and `model_router` refuses it whether the count is one or two — so one slot bought the floor nothing. What it cost is at the top of the band: `standard` runs to 24GB, where half is 12GB, the pair fits, and a host with room for two was narrowed to a shared model and told "this profile keeps one model resident". `tests/test_vision_role_resolution.py::test_a_standard_host_with_room_keeps_its_reader` fails CI on that narrowing; the 32GB test beside it is `performance` and passes at either setting, which is why nothing caught it.

**A fourth site sizes the machine, and it is Rust.** `install.sh`, `install.ps1` and `bootstrap.sh` are read by `tests/test_installer_models.py`; `src-tauri/src/supervisor.rs` sizes the drag-installed DMG, which has no install step to ask in, and no Python test could see it. When the floor moved to 16GB the three shell sites followed and the shell did not, leaving `gb >= 24 => 2, _ => 1`: a 24GB Mac running the bundled app gave Ollama one slot while the backend resolved a text model plus a reader against it — the pair chosen, then evicted on every switch, which is I-31's disagreement in its worst direction and I-37's load cost paid repeatedly. `tests/test_installer_models.py::test_the_desktop_shell_agrees_about_the_residency_band` reads the Rust and fails CI if its band drifts from `PROFILE_MIN_RAM_GB` again.
...
`_parse_teachback_response` returns whatever the model wrote, and `misconceptions` came back as `[["Concepts are just tags…", "stored in a list rather than a graph"]]` on a real card. Nothing downstream survived it, in two places that look unrelated. The JSON column stored the nested list, so `TeachbackResultItem` — which declares `list[str]` — failed validation and `GET /study/teachback/results` answered **500**; the docked Practice panel polls every card of a run in one batched call, so one malformed card erased the verdicts of every card beside it, and the learner watched a finished run report nothing. Separately, `MisconceptionModel.correction_note` is a string column and SQLite refused to bind a list to it (`Error binding parameter 6: type 'list' is not supported`), which before this change ran inside the same transaction as the result row and rolled the score back to `status="error"` — so the visible symptom was a card that scored and then unscored itself.
...
Folding the teach-back rubric into the evaluation call removed a second LLM call that was grading the learner against the card's own answer rather than the passage. The merged prompt asked for three integers *and* a one-sentence `clarity_comment`, and the sentence was the whole cost. Measured end to end against the unmerged build on the same library, same cards, same explanation: **4/16 submissions scored against 14/16** — a card that failed to parse is a learner who wrote an explanation and got nothing back. In isolation the same prompt parsed 12/12 and 9/9, so a harness run is not evidence here; only the paired servers showed it. The failures were one family, visible only once the log stopped truncating the raw reply at 200 characters:

```
"clarity": 50, "clarity_comment": The explanation is clear but contradicts the passage.}
                                  ^ no opening quote
```

Three of six failures dropped that opening quote. With `clarity_comment` removed and the three integers kept, two independent runs put the merged call at **20/20 and 18/20 scored against 17/20 and 18/20** — parse rate held — while the median wait fell from 12.3s to 6.1s and rubric coverage rose from 5/20 to 20/20, because the separate rubric call had been failing ~75% of the time unnoticed. Numbers after prose are safe; prose after numbers is not. The rule is recorded next to `_TEACHBACK_USER_TMPL`: no free-text field joins that call without re-running the parse-rate arm, not just the latency arm.
...
`POST /flashcards/generate` reads the passage its scope names and nothing else, so a second call on the same scope reads the same text and writes the same questions — which the near-duplicate filter then removes, every one of them. The reader's Practice face offered "add more questions", waited, and answered *"No cards came back. There may be no unused material left here."* on a document with **18 of its 26 passages untouched**. `regenerate` had solved this already and generate had not: `_passage_not_yet_used` exists precisely to sweep forward through a document, and only the replace path was passing it anything.

Measured on the reported document, by the chunks the delivered cards recorded: without the exclusion, both cards came from `[1, 2, 9, 10, 11, 13, 17, 18]` — **exactly the eight chunks the existing deck was written from**, and one of them re-asked a question already in the deck. With it, all three came from `[0, 3, 4, 5, 6, 7, 8, 12]`, every one unread. `avoid_used_material` is opt-in rather than the default because a caller naming a passage means that passage, and it is ignored when a selection is supplied — the selection *is* the passage, and excluding used chunks there would refuse to write a card about text the reader just highlighted.

The button is the other half. `GET /flashcards/{id}/headroom` reports what no card has been written from, and the panel hides "add more" rather than offering work that cannot succeed. Three cases are deliberately **not** exhausted, because each would hide a control that works: a failed request (an unanswered question is not a no), a document with no chunks (unreadable, not covered), and cards predating `source_chunk_ids` (they claim no passage, so the count is an upper bound — it can over-state what is left, never under-state it, and the empty-result message remains for that case). `tests/test_material_headroom.py` fails CI if generate stops excluding used material, if a selection starts being excluded, or if the headroom under-states what is left; `practiceDeck.test.ts` guards the three not-exhausted cases.

**I-45. An evaluator is graded on the wrong thing three ways at once: it must be told the question, given only the passage the card came from, and never asked for a headline beside the breakdown that explains it.**
...
**It was never told the question.** `_TEACHBACK_USER_TMPL` interpolated the passage, the card's answer and the explanation, and nothing else — so `completeness`, defined as "how much of what the passage covers the student said", was measured against the whole passage. On a card asking *why personal context matters*, the stored `missing_points` were "creating a personalized workflow definition (agents.md)" and "demonstrating the iterative checking process with /skeptical": two topics the question did not raise. Completeness 10/100 for answering exactly what was asked. The question is in the prompt now, and both `completeness` and `missing_points` are scoped to it.

**And "the passage" was the whole generation batch's.** `source_chunk_ids` records the chunks that were in the *prompt*, so every card from one call carries the same list — and over `_CHUNK_CHAR_LIMIT` that list is windows sampled from across the document (`_build_text`). One card's recorded passage was chunks 12, 20, 28 and 29 of a 40-chunk transcript: four places covering multiplayer agents, `agents.md`, `/skeptical` and career anxiety, joined with a blank line and no seam. Measured over the 171 cards in this library whose quote can be located, a recorded passage runs to 6.5 chunks against the 3.7 the card was written from — **38% of the graded text is material the card did not come from**, and 41% of those passages jump across the document. The card's own quote picks its window out, because the generation gate has already checked that quote is verbatim: `contiguous_runs` groups the recorded chunks into runs of consecutive `chunk_index` and `run_containing` returns the one whose text really holds the quote. Runs, not single chunks, because chunking cuts sentences and the chunk either side of a quote is its context — counting only the chunk holding the quote reads 81% foreign and would throw that context away. Where the quote locates nothing the whole recorded passage is kept with its seams marked `[...]`: grading against too much beats grading against nothing, but an evaluator may not read across a gap it cannot see. `_card_chunk_ids` now records the run at generation, so new cards need no repair -- and a chunk that was in a prompt but produced no card stops counting as used, which is what it always was, so `material_headroom` (I-44) offers that passage again rather than calling the document spent.

**And the headline was a fourth integer asked for beside the three dimensions, with nothing relating them.** Across the 90 stored verdicts carrying a breakdown, **24 (27%) have a headline outside the range their own two dimensions bracket** — no weighting of accuracy and completeness produces `score` 85 under 95/90, or `score` 75 over 60/50 — and **5 learners were told "Good explanation!" and had FSRS schedule the card as known while both dimensions failed**. The panel prints the breakdown directly beneath the headline, so a reader takes one for the explanation of the other; they were two opinions. `_score_from_dimensions` computes it: `0.6*accuracy + 0.4*completeness`, with a dimension below `_DIMENSION_FLOOR` unable to reach a pass. Accuracy leads because saying something the passage does not support is worse than stopping short. **Clarity carries no weight** — it is the one dimension with no passage behind it, and it read 6/100 on an explanation whose own `correct_points` named two right concepts; a number produced out of taste may describe an answer and may not grade one. It is still shown, labelled as not counted.

The prompt keeps its three integers **last** (I-43): the score is derived, but the parse rate that ordering bought is not. Re-measured on paired servers, alternating which arm goes first, over 40 submissions of real explanations: **34/40 scored before, 40/40 after**, median wait 7.3s to 4.9s and 6.7s to 5.9s across the two runs. What those runs do **not** establish is that the scores themselves are better: per-case spread on a 4B model dominates -- one explanation scored 10, 45 and 70 on the old arm and 40, 52 and 84 on the new -- and the two runs disagreed on which arm separates real explanations from another document's text further apart (31.0 against 15.6, then 20.3 against 28.8). The case for this change is what reaches the evaluator, which is deterministic and measured above, not a score delta that two runs could not agree on. `tests/test_teachback_passage_scope.py` fails CI if a card is graded against another card's window or a discontinuous passage loses its seams; `test_teachback_grounding.py` if the question leaves the prompt or the model is asked for a score again; `test_teachback_rubric.py::test_score_from_dimensions` if the headline stops being a function of the breakdown, and `latestAttempts.test.ts` if the panel tells a learner nothing was missing while scoring them 5.
...
The docked Practice header reads "N of M reviewed". It has been wrong twice, from two different counters, and the second one shipped *after* the first was fixed and guarded.

The first read **"4 of 3 reviewed"**: the in-run tally incremented per submission, so a card answered again counted twice. That became `countedCards`, a set of ids, and a `verify-dock` check on the summary readout.

The second read **"30 of 15 reviewed"** on a real session holding 30 teach-back attempts across 5 cards. Resuming a continuous run does not use that tally at all — `studySessionService.reattach` seeds the count from `Math.max(prevResults.length, remaining.answered_count)`, and those two terms are in **different units**: `GET /sessions/{id}/remaining-cards` already returns `answered_count` as a set of flashcard ids restricted to the plan, while `prevResults` is one row per *attempt*. The max then reliably picks the wrong one. Three lines below, `onResumeLoaded` seeds `countedCards` with `new Set(prev.map((r) => r.flashcard_id))` — the same question, answered correctly, in the same function. Neither site reads as wrong on its own; only together do they.

The max itself is load-bearing and must stay: `answered_count` is restricted to `planned_card_ids`, so a card answered and then regenerated out of the plan would vanish from the tally without it. Both terms just have to be distinct cards.

A sweep for the rest of the class found a third site: `end_session` tallied the recall arm as `cards_reviewed = len(events)` — review events, not cards — so a card graded twice in a sitting counted twice there too. `_latest_event_per_card` is the recall arm's twin of `_latest_attempt_per_card`, and the grade that stands is the later one.

**A continuous run makes this class inevitable**, which is why the guard is structural rather than per-counter: one session per document per mode (I-44's sibling decision) accumulates attempts across sittings indefinitely, so any counter that grows per submission diverges from the plan the longer the run is kept open — and it diverges silently, because both numbers look plausible until one passes the other. `verify-dock`'s `checkProgressHeader` parses the rendered ratio and fails if the numerator exceeds the denominator, and it runs on a fresh run *and* on a resumed one; `studySessionService.test.ts` fails CI if the resume tally counts rows again, and if the max stops covering a result whose card has left the plan.
...
Reported from the reader as "7 of 8 reviewed" on a document holding **three** cards. Replacing the deck deletes the cards an open run planned, and `planned_card_ids` keeps their ids; their `review_events` stay behind too, because an event records a review that happened (I-23's sibling: the repair job deletes index rows, never events). `GET /sessions/{id}/remaining-cards` then counted those five ids as answered, dropped them from `cards` because no row loads, and returned `planned_count` over the raw plan — so `answered + remaining == planned` and the inflation was invisible to every reader. The client cannot repair this: `reattach` reconciles with `Math.min(planned_count, answered + live remaining)`, and both terms inflate together.
...
Two things made this survive the sweep that produced I-46. It is not an arithmetic error — 7 ≤ 8, every counter agreed with itself, and the ratio-vs-plan check passed. And the harness read its denominator from the same endpoint that produced the header, so the system was verifying itself (`product-integrity.md`); `verify-dock` now takes the cap from `GET /flashcards/{document_id}`, which the header is not built from, and fails when a run counts more cards than the deck holds. `test_practice_run_tally.py::test_a_replaced_deck_leaves_the_run_no_phantom_progress` fails CI if a deleted card counts on either side.

The client-side half: `endOpenSessionsForScope` closed only the newest open run per mode, because `GET /sessions/open` returns one row. A second open run on the same document then survived a replacement, was adopted by the panel, and had the fresh cards appended to its dead plan — which is how the run above came to hold five deleted cards and three live ones.

**I-48. One generation call may not ask the same thing twice, and the answer is what shows it — but only within that call. Widening the same test to the deck was measured and refuses a fifth of every document's cards.**
A reader replaced a document's deck and got three cards, two of which asked the same thing: *"Why does rambling while prompting provide more value to an AI than concise input?"* beside *"What advantage does providing continuous context via ramble offer compared to a linear list of instructions?"* The dedup check ran, measured them at **0.8014**, and passed them: its bar is 0.85. Across the 221 real generation calls in this library (549 cards written by a call that produced more than one), the question bar has caught a within-call repeat **zero times** — it is not a check on what one call produces.

**The second axis is the answer.** The pair above scores 0.7614 on answers, so `_repeats_this_call` refuses a candidate that clears question ≥ 0.78 **and** answer ≥ 0.75 against a card already accepted from the same call. Both bars are bracketed by real cards named next to the constants: the ramble pair at 0.8014/0.7614 is refused, and "Why do massive-scale corpora require hard choices regarding storage costs?" against "Why are lexical search and dense retrieval considered complementary?" at 0.7890/0.7410 is kept.

**The scope is the measured part, and it must not be widened.** Applied to the whole existing deck the identical bars refuse a **median 19.1%** of every document's cards (308 of 947, against 105 for the question bar alone), and the sample is full of plainly distinct cards — "Why is it necessary to define a label for the nodes in an abstract syntax tree?" scores 0.785/0.809 against "How does the `_backward` method enable automatic differentiation". A deck accumulates legitimately similar cards over months of separate runs; one call reading one passage does not. Within a call the same rule costs **21 of 549 cards (3.8%)**. Do not widen it, and do not lower the deck's 0.85 to reach these cases either: **1,396 pairs** in this library sit in [0.80, 0.85) on questions alone and most are different cards. Dedup drops are not backfilled, so every point of over-rejection is paid in cards the learner never receives.

**And the check could not fire at all on an empty deck.** Candidate questions were encoded only `if candidates and existing_vecs is not None`, and an accepted candidate joins the call's pool — so comparing a batch against *itself* depended on there already being cards. The first generation on a document, the one run with nothing to compare against, was the only run that could hand back the same question twice (`product-integrity.md`: a check that cannot fail is not a check). `test_flashcard_duplicate_rule.py` fails CI if either bar moves without its bracketing cases, if the joint test reaches the deck, or if a first batch stops being checked against itself.
