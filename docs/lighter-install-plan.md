...
| `prune_appimage.sh` removes `libwayland-*`, `libcuda`, `libnvidia-ml`, `libvulkan` after the build (tauri's linuxdeploy ignores `LINUXDEPLOY_EXCLUDED_LIBRARIES`) | linuxdeploy resolves every ELF in the AppDir; driver libraries must come from the host | `check_appimage_host_libs.sh` on four distros, and the Fedora Wayland launch |
