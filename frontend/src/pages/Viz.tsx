...
export default function Viz() {
  ...
  // Prerequisites layer toggle: show/hide PREREQUISITE_OF edges
  const [showPrerequisites, setShowPrerequisites] = useState(true)
  // Cross-book layer toggle: show/hide SAME_CONCEPT edges -- default off (noisy)
  const [showCrossBook, setShowCrossBook] = useState(false)
  // Notes layer toggle: show/hide Note nodes -- default off
  ...
  // Detect whether selected document is a code document
  ...
  // Reset call_graph mode when the selected document is no longer a code document
  useEffect(() => {
  ...
  const filteredGraph = useMemo(() => {
    ...
    // Cluster view: when enabled and non-note entity count > 200
