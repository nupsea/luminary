...
function MasteryPanel({ documents }: { documents: Document[] }) {
  const [selectedDocId, setSelectedDocId] = useState<string>(documents[0]?.id ?? "")
  const [conceptsState, setConceptsState] = useState<SectionState<MasteryConceptsResponse | null>>(
    initSection(null),
  )
  const [heatmapState, setHeatmapState] = useState<SectionState<MasteryHeatmapResponse | null>>(
    initSection(null),
  )
    ...
    <div className="flex flex-col gap-6">
        ...
        {heatmapState.loading ? (
          ...
          <div className="overflow-auto rounded-lg border border-border">
            <table className="border-collapse text-xs">
              <thead>
                <tr>
                  <th className="min-w-24 bg-secondary px-3 py-2 text-left text-muted-foreground">Concept \ Chapter</th>
                  {heatmap.chapters.map((ch) => (
                    <th key={ch} className="min-w-20 bg-secondary px-2 py-2 text-center text-muted-foreground" title={ch}>
      ...
      <section className="flex flex-col gap-3">
        ...
        {conceptsState.loading ? (
          ...
          <div className="flex flex-col gap-2">
            {weakSpots.map((c) => (
              <div key={c.concept} className="flex items-center justify-between rounded-lg border border-border bg-card px-4 py-3">
      ...
      {noCards.length > 0 && (
        <section className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            {noCards.map((c) => (
              <span key={c.concept} className="rounded-full border border-border bg-secondary px-3 py-1 text-xs text-muted-foreground">
                {c.concept}
              </span>
            ))}
          </div>
  ...
  return (
    <div className="flex flex-col gap-8 px-6 py-8">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-foreground">Dev Tools</h1>
        {/* Tab bar */}
